...
namespace goog.debug{
  export class Trace_{
    ...
    constructor(){
       ...
       */
      this.traceCallbacks_ = <goog.debug.Trace_.TracerCallbacks>{};
    }
     ...
     */
    public logger_ = goog.log.getLogger('goog.debug.Trace');
     ...
     */
    public addTraceCallbacks(callbacks) {
     ...
     */
    public initCurrentTrace(defaultThreshold) {
     ...
     * @private
     */
    public releaseEvents_() {
      for (var i = 0; i < this.events_.length; i++) {
        var event:any = this.events_[i];
        if (event.id) {
     ...
     */
    public startTracer(comment, opt_type?) {
     ...
     */
    public stopTracer(id, opt_silenceThreshold?) {
...
namespace goog.debug{
   ...
   */
  export var Trace: goog.debug.Trace_ = new goog.debug.Trace_();
}
