...
namespace goog.math{
   ...
   * @template S,T
   */
  export function longestCommonSubsequence<S, T>(array1: IArrayLike<S>, array2: IArrayLike<T>, opt_compareFn: Function | undefined, opt_collectorFn: Function | undefined): Array<S | T> {
  ...
  export function sum(var_args: number[]): number {
    return /** @type {number} */ (goog.array.reduce(arguments, function (sum:number, value:number) { return sum + value; }, 0));
  };
  ...
  export function sampleVariance(var_args: number[]): number {
    ...
    var variance = goog.math.sum.apply(null, goog.array.map(arguments, function (val:number) {
