...
namespace goog.fs{
  ...
  export class FileReader extends goog.events.EventTarget{
     ...
     * @private
     */
    public dispatchProgressEvent_(event: ProgressEvent) {
     ...
     */
    public readAsBinaryString(blob: Blob) {
     ...
     */
    public readAsText(blob: Blob, opt_encoding?: string | undefined) {
     ...
     * @param {!Blob} blob The blob to read.
     */
    public readAsDataUrl(blob) {
      this.reader_.readAsDataURL(blob);
    };
  }
}

namespace goog.fs.FileReader {
  /**
   ...
   *     If an error occurs, the errback is called with a {@link goog.fs.Error}.
   */
  export function readAsText(blob, opt_encoding) {
    var reader = new goog.fs.FileReader();
    var d = goog.fs.FileReader.createDeferred_(reader);
   ...
   * @private
   */
  export function createDeferred_(reader) {
