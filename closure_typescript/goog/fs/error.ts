...
namespace goog.fs{
  ...
  export class Error extends goog.debug.Error{
    ...
    constructor(error, action?) {
      super(goog.string.subs('%s %s', '', action));
      /** @type {string} */
      this.name;
...
namespace goog.fs.Error{
   ...
   * @private
   */
  export function getNameFromCode_ (code) {
   ...
   * @private
   */
  export function getCodeFromName_ (name) {
   ...
   */
  export var NameToCodeMap_ = goog.object.create(
