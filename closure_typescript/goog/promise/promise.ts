...
namespace goog{
   ...
   */
  export class Promise<TYPE,RESOLVER_CONTEXT> implements goog.Thenable<TYPE>{
     ...
     * @final
     */
    constructor(resolver, opt_context?: RESOLVER_CONTEXT | undefined) {
      /**
    ...
    public thenVoid(
       ...
       * @template THIS
       */
      public thenAlways(onSettled, opt_context) {
        if (goog.Promise.LONG_STACK_TRACES) {
       ...
       * @template THIS
       */
      public thenCatch(onRejected, opt_context) {
        if (goog.Promise.LONG_STACK_TRACES) {
       ...
       * @private
       */
      public cancelChild_(childPromise, err) {
       ...
       * @private
       */
      public addCallbackEntry_(callbackEntry) {
       ...
       * @private
       */
      public unblockAndFulfill_(value) {
       ...
       * @private
       */
      public unblockAndReject_(reason) {
        goog.asserts.assert(this.state_ == goog.Promise.State_.BLOCKED);
        this.state_ = goog.Promise.State_.PENDING;
       ...
       * @private
       */
      public removeEntryAfter_(previous) {
       ...
       * @private
       */
      public removeUnhandledRejection_() {
        if (goog.Promise.UNHANDLED_REJECTION_DELAY > 0) {
...
namespace goog.Promise{
  export var LONG_STACK_TRACES = false;
  export var UNHANDLED_REJECTION_DELAY = 0;
  /**
   ...
   * @private
   */
  export function getCallbackEntry_(onFulfilled, onRejected, context) {
   ...
   * @private
   */
  export function returnEntry_(entry) {
   ...
   * =:
   */
  export function resolve(opt_value) {
   ...
   *     given reason.
   */
  export function reject(opt_reason) {
   ...
   * @private
   */
  export function resolveThen_<TYPE>(value: (goog.Thenable<TYPE> | Thenable<TYPE> | TYPE) | null, onFulfilled: (arg0: TYPE) =>any, onRejected: (arg0: any) => any) {
    var isThenable = goog.Promise.maybeThen_(value, onFulfilled, onRejected, null);
    if (!isThenable) {
      goog.async.run(<(this: Function) => void>goog.partial(onFulfilled, value));
    }
  };
  ...
  export function all(promises) {
    return new goog.Promise(function(resolve, reject) {
      ...
      for (var i = 0, promise; i < promises.length; i++) {
        promise = promises[i];
        goog.Promise.resolveThen_(promise, <(arg0: any) => any>goog.partial(onFulfill, i), onReject);
      }
    });
  };
    ...
    return new goog.Promise(function(resolve, reject) {
      ...
      for (var i = 0, promise; i < promises.length; i++) {
        ...
        goog.Promise.resolveThen_(
            promise, <(arg0: any) => any>goog.partial(onSettled, i, true /* fulfilled */),
            <(arg0: any) => any>goog.partial(onSettled, i, false /* fulfilled */));
      }
    });
  };
  ...
  export function firstFulfilled(promises) {
    return new goog.Promise(function(resolve, reject) {
      ...
      for (var i = 0, promise; i < promises.length; i++) {
        promise = promises[i];
        goog.Promise.resolveThen_(promise, onFulfill, <(arg0: any) => any>goog.partial(onReject, i));
      }
    });
  };
   ...
   * @private
   */
  export function maybeThen_(value, onFulfilled, onRejected, context) {
   ...
   * @private
   */
  export function invokeCallback_(callbackEntry, state, result) {
   ...
   * @private
   */
  export function addUnhandledRejection_(promise, reason) {
    if (goog.Promise.UNHANDLED_REJECTION_DELAY > 0) {
   ...
   * @private
   */
  export var handleRejection_ = goog.async.throwException;
...
namespace goog.Promise{
  ...
  export class Resolver_<TYPE>{
     ...
     * @template TYPE
     */
    constructor(promise, resolve, reject: () => void) {
      /** @const */
