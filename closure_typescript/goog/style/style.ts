...
namespace goog.style{
   ...
   * @private
   */
  export function setStyle_(element, value, style) {
   ...
   * @private
   */
  export function getStyle_(element, style) {
   ...
   * @private
   */
  export function getBoundingClientRect_(el) {
    var rect;
    try {
   ...
   * @private
   */
  export function getClientPositionForElement_(el) {
    var box = goog.style.getBoundingClientRect_(el);
   ...
   * @private
   */
  export function getPixelStyleValue_(value, round) {
   ...
   * @private
   */
  export function evaluateWithTemporaryDisplay_(fn, element) {
   ...
   * @private
   */
  export function getSizeWithDisplay_(element) {
   ...
   * @private
   */
  export var unselectableStyle_ = goog.userAgent.GECKO ?
      'MozUserSelect' :
   ...
   * @private
   */
  export function setBoxSizingSize_(element, size, boxSizing) {
    var style = element.style;
   ...
   * @private
   */
  export function getIePixelValue_(element, value, name, pixelName) {
   ...
   * @private
   */
  export function getIePixelDistance_(element, propName) {
   ...
   * @private
   */
  export function getBox_(element, stylePrefix) {
    if (goog.userAgent.IE) {
      ...
      let left = goog.style.getComputedStyle(element, stylePrefix + 'Left');
      let right = goog.style.getComputedStyle(element, stylePrefix + 'Right');
      let top = goog.style.getComputedStyle(element, stylePrefix + 'Top');
      let bottom = goog.style.getComputedStyle(element, stylePrefix + 'Bottom');
   ...
   * @private
   */
  export var ieBorderWidthKeywords_ = {
    'thin': 2,
    'medium': 4,
    'thick': 6
  };
   ...
   * @private
   */
  export function getIePixelBorder_(element, prop) {
  ...
  export function getBorderBox(element) {
    if (goog.userAgent.IE && !goog.userAgent.isDocumentModeOrHigher(9)) {
      ...
      // On non-IE browsers, getComputedStyle is always non-null.
      let left = goog.style.getComputedStyle(element, 'borderLeftWidth');
      let right = goog.style.getComputedStyle(element, 'borderRightWidth');
      let top = goog.style.getComputedStyle(element, 'borderTopWidth');
      let bottom = goog.style.getComputedStyle(element, 'borderBottomWidth');
  ...
  export function getFontFamily(el) {
    var doc:any = <Document>goog.dom.getOwnerDocument(el);
    var font = '';
   ...
   * @private
   */
  export var ABSOLUTE_CSS_LENGTH_UNITS_ = {
    'cm': 1,
    'in': 1,
    'mm': 1,
    'pc': 1,
   ...
   * @private
   */
  export var CONVERTIBLE_RELATIVE_CSS_UNITS_ = {
    'em': 1,
    'ex': 1
  };
  ...
  export function parseStyleAttribute(value: string): object {
    var result = {};
    goog.array.forEach(value.split(/\s*;\s*/), function (pair:string) {
  ...
  export function getScrollbarWidth(opt_className?) {
    ...
    var outerDiv = <HTMLDivElement>goog.dom.createElement(goog.dom.TagName.DIV);
    if (opt_className) {
      outerDiv.className = opt_className;
    }
    outerDiv.style.cssText = 'overflow:auto;' +
        'position:absolute;top:0;width:100px;height:100px';
    var innerDiv = <HTMLDivElement>goog.dom.createElement(goog.dom.TagName.DIV);
   ...
   * @private
   */
  export var MATRIX_TRANSLATION_REGEX_ = new RegExp(
      'matrix\\([0-9\\.\\-]+, [0-9\\.\\-]+, ' +
      '[0-9\\.\\-]+, [0-9\\.\\-]+, ' +
      '([0-9\\.\\-]+)p?x?, ([0-9\\.\\-]+)p?x?\\)');
