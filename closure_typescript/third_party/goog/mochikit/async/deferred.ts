...
namespace goog.async{
   ...
   * @template VALUE
   */
  export class Deferred<VALUE> implements goog.Thenable<VALUE>{
     ...
     * @private
     */
    public continue_(isSuccess, res) {
      this.blocked_ = false;
     ...
     * @private
     */
    public updateResult_(isSuccess, res) {
      this.fired_ = true;
      this.result_ = res;
     ...
     * @private
     */
    public makeStackTraceLong_(error) {
      if (!goog.async.Deferred.LONG_STACK_TRACES) {
        return;
      }
     ...
     * @template T
     */
    public addCallback(cb, opt_scope?) {
     ...
     * @template T
     */
    public addErrback(eb, opt_scope) {
     ...
     * @template T
     */
    public addCallbacks(cb, eb, opt_scope?) {
     ...
     * @override
     */
    public then(opt_onFulfilled, opt_onRejected,
        opt_context) {
      var resolve, reject;
     ...
     * @return {!goog.async.Deferred} This Deferred.
     */
    public chainDeferred(otherDeferred) {
      this.addCallbacks(
     ...
     * @return {!goog.async.Deferred} This Deferred.
     */
    public awaitDeferred(otherDeferred) {
     ...
     */
    public branch(opt_propagateCancel) {
      var d = new goog.async.Deferred();
...
namespace goog.async.Deferred{
    export var STRICT_ERRORS = false;
...
namespace goog.async.Deferred{
   ...
   */
  export function succeed(opt_result) {
    var d = new goog.async.Deferred();
   ...
   * @template T
   */
  export function when(value, callback, opt_scope) {
...
namespace goog.async.Deferred{
   ...
   * @private
   */
  export function scheduleError_(error) {
   ...
   * @private
   */
  export function unscheduleError_ (id) {
